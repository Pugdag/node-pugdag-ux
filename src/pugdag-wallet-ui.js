...
import {
	Deferred, GetTS, PUG, formatForMachine, formatForHuman,
...
import {initPugdagFramework, Wallet, workerLog} from '@pugdag/wallet-worker';
export {Wallet};
Wallet.setWorkerLogLevel(localStorage.walletWorkerLogLevel || 'none')

export {html, css, FlowFormat, dpc, baseUrl, debug};

export class PugdagWalletUI extends BaseElement{
	...
	async initNetworkSettings() {
		...
		//const { network, port } = this.local_pugdagd_settings;
			...
			<div class="tx-notifications">
				${notifications.map(n=>{
					return html`<div class="tx-notification">
						${n.compoundUTXOs?
							T(`Compounding UTXOs...`):
							i18n.t(`Preparing transaction for [n] PUG ....`)
							.replace('[n]', this.formatPUG(n.amount))}
					</div>`
				})}
				...
				return html`
					<flow-expandable class="tx-row" static-icon expand ?txin=${tx.in} ?txmoved=${tx.isMoved} ?txout=${!tx.in}
						...
						<div class="tx-title" slot="title">
							...
							<div class="amount">
								${tx.in?'':'-'}${this.formatPUG(tx.amount)} PUG
							</div>
						</div>
						...
						<div class="tx-body">
							${tx.note}
							<div class="tx-id">
								<a target="_blank" href="https://explorer.pugdag.com/txs/${tx.id}">${tx.id}</a>
							</div>
							<div class="tx-address">
								${tx.myAddress?T('COMPOUNDING WALLET => '):''}
								<a target="_blank" href="https://explorer.pugdag-com/addresses/${tx.address}">${tx.address}</a>
							</div>
						</div>
					</flow-expandable>
				`
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?txin=${tx.in} tx-version=${tx.version} ?txmoved=${tx.isMoved} ?txout=${!tx.in}>
						...
						<div class="tx-amount">${tx.in?'':'-'}${PUG(tx.amount)} PUG</div>
						<div class="br tx-note">${tx.note}</div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.pugdag.com/txs/${tx.id.split(":")[0]}">${tx.id.split(":")[0]}</a>
						</div>
						<div class="tx-address">
							${tx.myAddress?T('COMPOUNDING WALLET => '):''}
							<a target="_blank" href="https://explorer.pugdag.com/addresses/${tx.address}">${tx.address}</a>
		...
		items.map((tx, i)=>{
			...
			fields.forEach(field=>{
				switch(field){
					...
					break;
					case 'amount':
						row.push(escape(`${tx.in?'':'-'}${PUG(tx.amount)}`))
					break;
					case 'direction':
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?iscoinbase=${!tx.isCoinbase}>
						...
						<div class="tx-amount">${PUG(tx.satoshis)} PUG</div>
						<div class="br tx-mass"></div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.pugdag.com/txs/${tx.txId}">${tx.id}</a>
						</div>
						<div class="tx-address">
							<a target="_blank" href="https://explorer.pugdag.com/addresses/${tx.address}">${tx.address}</a>
						</div>
	...
	getWalletInfo(wallet) {
		...
		return new Promise((resolve, reject) => {
			...
			wallet.on("balance-update", ({ confirmedUtxosCount }) => {
				console.log("Événement balance-update déclenché");
				console.log("Balance actuelle - Disponible:", this.wallet.balance.available, ", En attente:", this.wallet.balance.pending, ", Total:", this.wallet.balance.total);
				console.log("confirmedUtxosCount:", confirmedUtxosCount);
	
				this.requestUpdate("balance", null);
	
				if (this.autoCompound) {
					console.log("Appel de compoundIfNeeded avec confirmedUtxosCount:", confirmedUtxosCount);
					this.compoundIfNeeded(confirmedUtxosCount);
				}
			});
			...
			wallet.on("state-update", ({ cache }) => {
				console.log("État de cache mis à jour:", cache);
			...
			wallet.on("scan-more-addresses-started", ({ receiveStart, changeStart }) => {
				console.log("Début du scan d'adresses - receiveStart:", receiveStart, ", changeStart:", changeStart);
				this.extraScanning = {
					status: "started",
					error: null,
					receiveStart,
	...
	async alertUTXOIndexSupportIssue(){
		...
		let body = html`${i18nHTMLFormat(`'utxoindex' flag is missing from PUGDAGD config.<br />
	...
	connectedCallback(){
		...
		let openDialog = document.createElement('pugdag-open-dialog');
		openDialog.hideLogo = !!this.hideOpenWalletLogo;
		this.parentNode.insertBefore(openDialog, this.nextSibling)
		this.sendDialog = document.createElement("pugdag-send-dialog"+mobileSuffix);
		this.parentNode.appendChild(this.sendDialog);
		this.receiveDialog = document.createElement("pugdag-receive-dialog"+mobileSuffix);
		this.parentNode.appendChild(this.receiveDialog);
		this.seedsDialog = document.createElement("pugdag-seeds-dialog");
		this.parentNode.appendChild(this.seedsDialog);
		let t9Dialog = document.createElement("pugdag-t9-dialog");
		this.parentNode.appendChild(t9Dialog);
		let qrscannerDialog = document.createElement("pugdag-qrscanner-dialog");
		qrscannerDialog.debug = this.debugscanner
		this.parentNode.appendChild(qrscannerDialog);
		let uploadFileDialog = document.createElement("pugdag-upload-file-dialog");
		this.parentNode.appendChild(uploadFileDialog);

		const {workerCorePath} = window.PugdagConfig||{}
		initPugdagFramework({
			workerPath: workerCorePath||"/pugdag-wallet-worker/worker.js?ident="+(window.PugdagConfig?.ident||"")
		}).then(()=>{
			let encryptedMnemonic = getLocalWallet()?.mnemonic;
	...
	async handleInitDialogCallback({dialog, password, seedPhrase, encryptedMnemonic}){
		...
		if(!rpc)
			return FlowDialog.alert(i18n.t("Error"), i18n.t("Pugdag Daemon config is missing."));
	...
	showTxDialog(){
		if(!this.txDialog){
			this.txDialog = document.createElement("pugdag-tx-dialog");
	...
	async isValidAddress(address){
		...
		if(window.mobileMode && prefix=="pugdagtest")
			return true;
	...
	*/
    ...
    //console.log("Amount (en Satoshis):", amount); 
    ...
    try {
        ...
        amountInSatoshi = new BN(amountStr);
        //console.log("Amount as BN before API call:", amountInSatoshi.toString());
        //console.log("Bit length of amountInSatoshi:", amountInSatoshi.bitLength());
    } catch (conversionError) {
        console.error("Error converting amount to BN:", conversionError);
        return { data: undefined, error: "Error converting amount to BN" };
    }
    ...
    if (fee) {
        try {
            ...
            //console.log("Fee as string:", feeStr);
            feeInSatoshi = new BN(feeStr);
            //console.log("Fee in Satoshi:", feeInSatoshi.toString());
        } catch (conversionError) {
            console.error("Error converting fee to BN:", conversionError);
            return { data: undefined, error: "Error converting fee to BN" };
        }
    } else {
        console.log("Fee in Satoshi: No fee");
    }

    const payload = {
        toAddr: address,
        amount: amountInSatoshi.toString(),
        fee: feeInSatoshi ? feeInSatoshi.toString() : undefined,
        calculateNetworkFee,
        inclusiveFee,
        note
    };
    ...
    try {
        const data = await this.wallet.estimateTransaction(payload);
        console.log("estimateTx result:", data);
        ...
        console.error("Error during estimateTransaction:", err);
	...
	async getPugdagFromFaucet(amount) {
	...
	requestFaucetFunds() {
		...
		showT9({
			...
			inputLabel:i18n.t('Amount in PUG')
		}, ({value:amount, dialog})=>{
			let sompis = formatForMachine(amount||0);
			if(sompis > this.faucetFundsAvailable){
				let msg = i18n.t(`You can't request more than [n] PUG.`)
						.replace("[n]", PUG(this.faucetFundsAvailable||0))
				return dialog.setError(msg);//'
			}
			
			dialog.hide();

			this.getPugdagFromFaucet(amount)
