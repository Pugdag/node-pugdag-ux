...
export class PugdagDialog extends BaseElement{
	...
	static get styles(){
		return [ScrollbarStyle, SpinnerStyle, css`
			:host{
				...
				position:var(--pugdag-dialog-position, absolute);
				top:0px;
				left:0px;
				width:100%;
				height:100%;
				background-color:var(--pugdag-dialog-bg-color);
			...
			.container{
				box-sizing:border-box;
				width:100%;
				height:var(--pugdag-dialog-container-height, calc(100% - 10px));
				background-color:var(--flow-background-color, #F00);
				z-index:1;
				border:var(--pugdag-dialog-container-border, 2px solid var(--flow-primary-color));
				border-radius:var(--pugdag-dialog-container-border-radius, 3px);
				max-width:var(--pugdag-dialog-container-max-width, 700px);
				max-height:var(--pugdag-dialog-container-max-height, 300px);
				margin:var(--pugdag-dialog-container-margin, 5px auto);
				padding:var(--pugdag-dialog-container-padding, 0px);
				position:relative;
			...
			:host([hideable]) .close-btn{
				display:var(--pugdag-dialog-container-close-btn-display, inline-block)
			}
			.heading{
			...
			[hidden]{display:none}
			.buttons{margin:var(--pugdag-dialog-buttons-margin, 10px auto);display:flex;width:var(--pugdag-dialog-buttons-width,90%);}
	...
	pushHistory(uid=""){
		...
		name = name.replace(/\-/g, "");
		let key = name.toLowerCase().replace(/(pugdag|dialog|mobile)/g, '')
