...
import {
	html, css, PugdagDialog, askForPassword, PUG,
	formatForMachine, getLocalWallet
} from './pugdag-dialog.js';
const pass = "";
import {Wallet} from '@pugdag/wallet-worker';

class PugdagSendDialog extends PugdagDialog{
	...
	static get styles(){
		return [PugdagDialog.styles, 
		css`
			.container{
				max-height:var(--pugdag-dialog-container-max-height, 670px);
			}
	...
	renderBody(){
		return html`
			<flow-input class="address full-width" outer-border
				label="${T(`Recipient Address (Must start with 'pugdag' prefix)`)}"
				value="${this.address||''}"
				placeholder="">
			</flow-input>
			<div col>
				<flow-input class="amount full-width" outer-border
					label="${T('Amount in PUG')}" @keyup=${this.onAmountChange}
					value="${this.amount}">
				</flow-input>
				<div spacer></div>
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${dataFee?html`<tr><td is="i18n-td">Data Fee</td><td>${PUG(dataFee)} PUG</td></tr>`:''}
				${fee?html`<tr><td is="i18n-td">Total Fee</td><td>${PUG(fee)} PUG</td></tr>`:''}
				${totalAmount?html`<tr is="i18n-td"><td>Total Amount</td><td> ${PUG(totalAmount)} PUG</td></tr>`:''}
			</table>
		</div>
		`
	}
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] PUG) is too large.');
			msg = msg.replace('[n]', PUG(estimate.fee));
...
PugdagSendDialog.define("pugdag-send-dialog");
