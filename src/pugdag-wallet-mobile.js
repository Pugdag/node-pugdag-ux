import {
	html, css, FlowFormat, PugdagWalletUI, dpc,
	baseUrl, PUG, renderPagination, buildPagination, paginationStyle,
...
} from './pugdag-wallet-ui.js';
export {isMobile, dontInitiatedComponent};
export class PugdagWalletMobile extends PugdagWalletUI{
	...
	static get styles(){
		return [PugdagWalletUI.styles, paginationStyle, swipeableStyle, css`
			:host{
			...
			.header{
				...
				padding:var(--pugdag-wallet-header-padding, 5px 10px);
				margin:var(--pugdag-wallet-header-margin, 0px);
			}
			...
			.pagination-box{
				padding:var(--pugdag-pagination-box-padding, 10px 5px;);
			}

			.logo{
				width:30px;height:30px;/*background-color:#DDD*/
				display:var(--pugdag-wallet-header-logo-diaplay, initial);
			}
			...
			.tabs-container{
				...
				border-top:2px solid var(--pugdag-wallet-tab-border-top-color, var(--flow-primary-color));
				border-bottom:1px solid var(--pugdag-wallet-tab-border-bottom-color, #DDD);
			}
			.tabs{
			...
			.tab.selected{
				border-bottom-color:var(--pugdag-wallet-tab-active-border-color, var(--flow-primary-color));
			}
			...
			.address-input{
				...
				min-width:var(--pugdag-wallet-address-input-min-width, 460px);
			...
			.send-scan-buttons a fa-icon{
				...
				background-color:var(--pugdag-wallet-scan-button-bg, #3d4e58);
				--fa-icon-color:var(--pugdag-wallet-scan-button-color, #FFF);
				box-shadow:var(--flow-box-shadow);
			}
			.send-scan-buttons .send-btn fa-icon{
				background-color:var(--pugdag-wallet-send-button-bg, #fb7470);
				--fa-icon-color:var(--pugdag-wallet-send-button-color, #FFF);
			}
			.send-scan-buttons .receive-btn fa-icon{
				background-color:var(--pugdag-wallet-receive-button-bg, #60b686);
				--fa-icon-color:var(--pugdag-wallet-receive-button-color, #FFF);
			}
			...
			.lock-screen{
				...
				position:absolute;
				z-index:1000000;
				background-color:var(--pugdag-lock-screen-bg-color);
				top:0px;bottom:0px;
				left:0px;right:0px;
			}
	...
	render(){
		...
		let donationAddresses = [
			["Pugdag Devfund donations:", "pugdag:qzrq7v5jhsc5znvtfdg6vxg7dz5x8dqe4wrh90jkdnwehp6vr8uj7csdss2l7"],
			["Pugdag WebWallet donations:", "pugdag:qqe3p64wpjf5y27kxppxrgks298ge6lhu6ws7ndx4tswzj7c84qkjlrspcuxw"],
		]
		...
		<div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
			<div class="flow-swipeable-row">
				...
				<div class="tab-content ${sCls('wallet')}" for="wallet">
					<div class="wallet-ux">
						<div class="badge" is="i18n-div">PugDAG WALLET</div>
						...
						<flow-expandable class="donation-info" no-info no-icon icon="-">
							...
							<p is="i18n-p">
								if you wish to further the development of the pugdag ecosystem, we accept donations at the following addresses:
							</p>
							${
						...
						<flow-expandable class="developer-info" _expand no-info no-icon icon="-">
							...
							<div class="badge"><span is="i18n-span">Pugdag Core PUG:</span> ${window.PWA_MODULES['@pugdag/core-lib']}</div>
							<div class="badge"><span is="i18n-span">Pugdag Wallet Framework PUG:</span> ${window.PWA_MODULES['@pugdag/wallet']}</div>
							<div class="badge"><span is="i18n-span">Pugdag gRPC:</span> ${window.PWA_MODULES['@pugdag/grpc']}</div>
							<div class="badge"><span is="i18n-span">Pugdag gRPC Relay:</span> ${window.PWA_MODULES['@pugdag/grpc-web']}</div>
							<div class="badge"><span is="i18n-span">Pugdag UX PUG:</span> ${window.PWA_MODULES['@pugdag/ux']}</div>
							<div class="badge"><span is="i18n-span">Flow UX PUG:</span> ${window.PWA_MODULES['@aspectron/flow-ux']}</div>
						</flow-expandable>
				...
				<div class="tab-content ${sCls('faucet')}" for="faucet">
					${this.faucetStatus ? this.faucetStatus : html`
						...
						<div class="faucet-ux">
							<div class="margin-bottom"  is="i18n-div">PUGDAG FAUCET</div>
							<div>${i18nFormat('Your IP is [n]', this.ip||"")}</div>
							<div class="margin">${i18nHTMLFormat('You have <b>[n] PUG</b> available.', PUG(this.faucetFundsAvailable||0) )}</div>
				...
				<div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
					<div class="info-ux">
						...
						<table>
							...
							<tr><td is="i18n-td">AMOUNT</td><td>${PUG(inUseUTXOs.satoshis||0)} PUG</td></tr>
						</table>
						<flow-btn class="center-btn primary v-margin"
	...
	renderLockScreen(){
		return html`
			<div class="lock-screen">
				<div><img class="big-logo" src="/resources/images/pugdag.png"></div>
	...
	openAddressExplorer(address){
		if (!address)
			return
		let url = `https://explorer.pugdag.com/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${ this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatPUG(total)+' PUG':''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatPUG(available)} PUG</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatPUG(pending)} PUG</span>
					</div>
				`}
            </div>
	...
	showReceiveDialog(){
		let address = this.receiveAddress||'pugdagtest:abc'
