...
import {
	html, css, PugdagDialog, PUG,
	formatForMachine, formatForHuman
} from './pugdag-dialog.js';

class PugdagT9Dialog extends PugdagDialog{
	...
	static get styles(){
		return [PugdagDialog.styles, css`
			.container{
				...
				max-height:var(--pugdag-dialog-container-max-height, 600px);
			}
	...
	renderBody(){
		...
		let {inputLabel='Amount in PUG'} = this;
		return html`
	...
	open(args, callback){
		...
		this.inputLabel = args.inputLabel||i18n.t('Amount in PUG');
		this.show();
	}
    cancel(){
    	this.hide();
...
PugdagT9Dialog.define("pugdag-t9-dialog");
