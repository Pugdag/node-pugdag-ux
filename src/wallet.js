...
import {helper, Storage, CONFIRMATION_COUNT, COINBASE_CFM_COUNT} from '@pugdag/wallet-worker';
export const {Deferred, PUG, Decimal} = helper;
export {CONFIRMATION_COUNT, COINBASE_CFM_COUNT};
const storage = new Storage({logLevel:'debug'});
let {baseUrl, debug, MAX_UTXOS_THRESHOLD=80, dontInitiatedComponent=false} = window.PugdagConfig || {};
if(!baseUrl){
	baseUrl = (new URL("../", import.meta.url)).href;
	debug && console.log("PugdagUX: baseUrl", baseUrl)
}
...
/*
const {Wallet, initPugdagFramework, Storage} = require("pugdag-wallet-worker");
let {Mnemonic} = Wallet;
...
export const {RPC} = require("pugdag-grpc-node");
*/
