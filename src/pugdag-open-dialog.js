import {html, css, PugdagDialog, i18n, T} from './pugdag-dialog.js';
import {askForPassword} from "./wallet.js";
const pass = "";

class PugdagOpenDialog extends PugdagDialog{
	...
	static get styles(){
		return [PugdagDialog.styles, css`
			.container{max-height:var(--pugdag-dialog-container-max-height, 600px)}
			:host([mode="create"]) .container{max-height:var(--pugdag-dialog-container-max-height, 500px)}
			:host([mode="init"]) .container{max-height:var(--pugdag-dialog-container-max-height, 200px)}
			:host([mode="recover"]) .container{max-height:var(--pugdag-dialog-container-max-height, 450px)}
			.buttons{justify-content:center;--pugdag-dialog-buttons-width:100%;}
	...
	renderInitUI(){
		return html`
			<div class="sub-heading text-center" is="i18n-div">Welcome to PugDAG Wallet</div>
		`
	}
	renderRecoverUI(){
		let rows = [0, 1, 2];
	...
	renderOpenUI(){
		...
		return html`
			${this.hideLogo?'': html`
			<div>
				<img class="big-logo" src="/resources/images/pugdag.png" />
			</div>`}
...
PugdagOpenDialog.define("pugdag-open-dialog");
