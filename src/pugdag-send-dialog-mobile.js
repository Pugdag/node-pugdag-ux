...
class PugdagSendDialogMobile extends PugdagDialog{
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${totalAmount?html`<tr><td is="i18n-td">Total Amount</td><td> ${PUG(totalAmount)} PUG</td></tr>`:''}
			</table>
		</div>
		`
	}
	...
	showT9(e){
		...
		showT9({
			value,
			heading:input.label.replace("in PUG", ""),
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] PUG) is very large.');
...
PugdagSendDialogMobile.define("pugdag-send-dialog-mobile");
